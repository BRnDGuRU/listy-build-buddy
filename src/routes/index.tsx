...
function CtaButton({ children = "Book a Strategy Call", inverse = false }: { children?: ReactNode; inverse?: boolean }) {
  return (
    <Button asChild size="lg" variant={inverse ? "secondary" : "default"} className="h-12 rounded-full px-7 font-bold shadow-soft">
...
function Logo() {
  return (
    <a href="#top" className="flex items-center gap-2 font-display text-lg font-extrabold text-foreground" aria-label="GTM Email home">
...
function HeroVisual() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-lg" aria-label="Managed email engine illustration">
      ...
      <div className="absolute inset-[34%] grid place-items-center rounded-full border border-primary bg-ink-card shadow-card">
        <Mail className="h-20 w-20 text-brand-light sm:h-28 sm:w-28" strokeWidth={1.25} />
      </div>
      {[
        { icon: Bot, label: "AI lead agent", className: "left-[6%] top-[14%]" },
        { icon: MessageSquareText, label: "Reply agent", className: "right-[2%] top-[28%]" },
        { icon: Database, label: "8M+ leads", className: "bottom-[12%] left-[8%]" },
        { icon: ServerCog, label: "Infrastructure", className: "bottom-[5%] right-[8%]" },
      ].map((item) => (
        <div key={item.label} className={`absolute ${item.className} flex w-28 flex-col items-center text-center`}>
          <span className="grid h-12 w-12 place-items-center rounded-full border border-ink-border bg-ink-elevated text-brand-light"><item.icon className="h-6 w-6" /></span>
          <span className="mt-2 text-[11px] font-bold uppercase text-ink-muted">{item.label}</span>
        </div>
      ))}
    </div>
  );
...
function Hero() {
  return (
    <section id="top" className="bg-ink py-16 text-primary-foreground sm:py-24">
      <div className="page-container grid items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
        <div>
          <span className="inline-flex rounded-full bg-primary px-4 py-2 text-xs font-extrabold uppercase text-primary-foreground">Done-for-you outbound GTM system</span>
          <h1 className="mt-7 max-w-3xl font-display text-5xl font-extrabold leading-[1.02] sm:text-6xl lg:text-7xl">
            100K GTM Email <span className="text-brand-light">Guaranteed System</span>
          </h1>
          <p className="mt-6 text-xl font-semibold sm:text-2xl">We build. We manage. <span className="text-brand-light">You get the meetings.</span></p>
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-ink-muted">A complete done-for-you outbound GTM engine that delivers more qualified appointments on autopilot.</p>
          <div className="mt-8"><CtaButton /></div>
          <div className="mt-10 grid max-w-2xl gap-5 border-t border-ink-border pt-7 sm:grid-cols-3">
...
function Proof({ icon: Icon, title, text }: { icon: typeof Target; title: string; text: string }) {
  return <div className="flex gap-3"><Icon className="mt-0.5 h-6 w-6 shrink-0 text-brand-light" /><div><strong className="block text-sm uppercase">{title}</strong><span className="mt-1 block text-xs leading-relaxed text-ink-muted">{text}</span></div></div>;
}
    ...
    <section id="included" className="section-space bg-background">
      <div className="page-container">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {inclusions.map((item, index) => <article key={item.title} className={`border border-border bg-card p-6 ${index === 0 || index === 10 ? "lg:col-span-2" : ""}`}><span className="grid h-11 w-11 place-items-center rounded-full bg-primary-soft text-primary"><item.icon className="h-5 w-5" /></span><h3 className="mt-5 font-display text-base font-extrabold uppercase">{item.title}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{item.text}</p></article>)}
        </div>
      </div>
    </section>
  );
...
function CommitmentSection() {
  return (
    <section className="bg-soft py-10">
      <div className="page-container">
        <div className="grid items-center gap-8 rounded-lg bg-ink p-7 text-primary-foreground sm:p-10 lg:grid-cols-[1fr_1.25fr]">
  ...
  return (
    <section id="investment" className="section-space bg-background">
      <div className="page-container">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{investments.map((item) => <article key={item.title} className="border-t-2 border-primary bg-soft p-6 text-center"><item.icon className="mx-auto h-8 w-8 text-primary" /><p className="mt-5 text-xs font-bold uppercase text-muted-foreground">{item.title} · {item.cadence}</p><strong className="mt-2 block font-display text-4xl text-primary">{item.price}</strong><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{item.text}</p></article>)}</div>
        <div className="mt-5 bg-ink px-6 py-8 text-center text-primary-foreground"><p className="text-xs font-bold uppercase text-ink-muted">Total on day 1</p><p className="mt-2 font-display text-5xl font-extrabold">$2,149</p><p className="mt-2 text-sm text-ink-muted">One-time activation + first month&apos;s running infrastructure + annual domains</p></div>
      </div>
    </section>
  );
}
...
function TimelineCard({ label, total, items }: { label: string; total: string; items: string[] }) {
  return <article className="border border-border bg-card p-7"><span className="inline-grid h-14 w-14 place-items-center rounded-full border border-primary font-display text-sm font-extrabold text-primary">{label}</span><ul className="mt-6 space-y-3">{items.map((item) => <li key={item} className="flex gap-2 text-sm"><CircleCheck className="h-4 w-4 shrink-0 text-primary" />{item}</li>)}</ul><div className="mt-7 border-t border-border pt-5"><span className="text-xs font-bold uppercase text-muted-foreground">Total</span><strong className="mt-1 block font-display text-2xl text-primary">{total}</strong></div></article>;
}
